...
<!-- This script has been abandoned in favor of the data_analysis.R script, which was imported from the now-deleted Entsoc repository. It is retained here because it contains code from the unsupervised classification, which will probably never be used but was too much work to throw away completely. You should be making all of your new updates in the data_analysis.R script. -->
...
<center>
![*Acyrthosiphon* aphids](graphics/aphid_image.jpg)
</center>
...
![](graphics/PIG_conceptualModel.png)
...
springTabs$Acyrthosiphon %>% kable(col.names = 
                                                 c('Intercept',
                                                   'Anthocoridae',
                                                   'Arachnida',
                                                   'Coccinellidae',
...
springTabs$Aphis %>% kable(col.names = 
                                                 c('Intercept',
                                                   'Anthocoridae',
                                                   'Arachnida',
                                                   'Coccinellidae',
                                                   ...
                                                   'delta',
                                                   'weight'), 
                                             caption = 'Spring - Aphis')

springTabs$Therioaphis %>% kable(col.names = 
                                                 c('Intercept',
                                                   'Anthocoridae',
                                                   'Arachnida',
                                                   'Coccinellidae',
...
fallTabs$Acyrthosiphon %>% kable(col.names = 
                                                 c('Intercept',
                                                   'Anthocoridae',
                                                   'Arachnida',
                                                   'Coccinellidae',
...
fallTabs$Aphis %>% kable(col.names = 
                                                 c('Intercept',
                                                   'Anthocoridae',
                                                   'Arachnida',
                                                   'Coccinellidae',
...
fallTabs$Therioaphis %>% kable(col.names = 
                                                 c('Intercept',
                                                   'Anthocoridae',
                                                   'Arachnida',
                                                   'Coccinellidae',
...
```{r aph_mod_vis}
## all out of date
sp_acy_topMod <- mean_density %>% 
  filter(Season == 'Spring') %$% 
  lmer(log(Acyrthosiphon + 1) ~
         log(Coccinellidae + 1) + 
         (1|Site) + (1|Field))

sp_aphis_topMod <- mean_density %>% 
  filter(Season == 'Spring') %$% 
  lmer(log(Aphis + 1) ~
         (1|Site) + (1|Field))

sp_therioaphis_topMod <- mean_density %>% 
  filter(Season == 'Fall') %$% 
  lmer(log(Therioaphis + 1) ~
         log(Geocoris + 1) + 
         (1|Site) + (1|Field))

fall_acy_topMod <- mean_density %>% 
  filter(Season == 'Fall') %$% 
  lmer(log(Acyrthosiphon + 1) ~
         log(Ichneumonidae + 1) +
         (1|Site) + (1|Field))

fall_aphis_topMod <- mean_density %>% 
  filter(Season == 'Fall') %$% 
  lmer(log(Acyrthosiphon + 1) ~
         log(Arachnida + 1) +
         log(Ichneumonidae + 1) +
...
fall_therioaphis_topMod <- mean_density %>% 
  filter(Season == 'Fall') %$% 
  lmer(log(Acyrthosiphon + 1) ~
         log(Arachnida + 1) +
         (1|Site) + (1|Field))

best_mod_list <- list(sp_acy_topMod, sp_aphis_topMod, sp_therioaphis_topMod,
                      fall_acy_topMod, fall_aphis_topMod, fall_therioaphis_topMod)
names(best_mod_list) <- c('sp_acy_topMod', 'sp_aphis_topMod', 'sp_therioaphis_topMod',
                      'fall_acy_topMod', 'fall_aphis_topMod', 'fall_therioaphis_topMod')

predicts <- map(best_mod_list, function(mod){plot_model(mod, type = 'pred')})
names(predicts) <- c('sp_acy_topMod', 'sp_aphis_topMod', 'sp_therioaphis_topMod',
                      'fall_acy_topMod', 'fall_aphis_topMod', 'fall_therioaphis_topMod')
effs <- map(best_mod_list, function(mod){plot_model(mod, type = 'est')})
names(effs) <- c('sp_acy_topMod', 'sp_aphis_topMod', 'sp_therioaphis_topMod',
                      'fall_acy_topMod', 'fall_aphis_topMod', 'fall_therioaphis_topMod')
  ...
  ui = fluidPage(
      ...
      selectInput(inputId = 'model', 'Model',
                  choices = list('Spring Acy' = 'sp_acy_topMod', 
                                 'Spring Aphis' = 'sp_aphis_topMod',
                                 'Spring Therioaphis' = 'sp_therioaphis_topMod',
                                 'Fall Acy' = 'fall_acy_topMod', 
                                 'Fall Aphis' = 'fall_aphis_topMod',
                                 'Fall Therioaphis' = 'fall_therioaphis_topMod')),
...
predlist <- c('Density_Arachnida', 'Density_Coccinellidae',
              'Density_Geocoris', 'Density_Ichneumonidae',
              'Density_Nabis', 'Density_Anthocoridae')
...
for (i in 1:length(predlist)) {
    ...
    predator <- as.name(predlist[[i]])
  sham_mods[[i]] <- lmer(eval(predator) ~ Treatment * Season + (1|Site), data = data)
  
  vars <- c('Intercept', 'Trt', 'Ssn', 'Trt*Ssn')
  confints <- confint(sham_mods[[i]])[3:6,]
  ests <- as_tibble(summary(sham_mods[[i]])$coefficients)
  sham_effs[[i]] <- as_tibble(cbind(vars, confints, ests))
